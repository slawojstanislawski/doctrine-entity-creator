...
class Strategy4 extends AbstractStrategy implements StrategyInterface
{
	...
	public function composeString()
	{
		...
		$methodString .= "\t\tif(!\$this->get" . ucwords($this->propertyName) . "()->contains(\$singleEntity)) {\n";
		$methodString .= "\t\t\t\$this->" . $this->propertyName . "->add(\$singleEntity);\n";
		$methodString .= "\t\t\t\$singleEntity->set" . ucwords($this->map) . "(\$this);\n";
		$methodString .= "\t\t}\n";
		...
		$methodString .= "\t\t\t\$singleEntity->set" . ucwords($this->map) . "(null);\n";
		$methodString .= "\t\t}\n";
		$methodString .= "\t\treturn \$this;\n";
		$methodString .= "\t}\n\n";
